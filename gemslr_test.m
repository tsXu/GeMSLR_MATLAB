...
for levi = (nlev-1):-1:1
    ...
    if (msteps > 0 && rank_k2 > 0)
        ...
        if m2 == 0
            ...
            % plot the eignevalue to see see
            if (ploteig)
                n_s = lev_ptr(levi);
                n_e = lev_ptr(levi+1)-1;
                I = eye(n-n_e);
                C   = A(n_e+1:n, n_e+1:n);
                S = C - PRE.Levs{levi}.E * (PRE.Levs{levi}.B \ PRE.Levs{levi}.F);
                matSCinv = I - (PRE.Levs{levi}.C - PRE.Levs{levi}.E * ( PRE.Levs{levi}.B \ PRE.Levs{levi}.F ) ) / C;
                eig_SCinv = eig(full(matSCinv));
                figure; plot(real(eig_SCinv), imag(eig_SCinv),'ro');
                hold on;
                
                if (msteps > 0 && local_rank > 0)
                    eig_Arnodi_H = eig(full(Arnodi_H));
                    plot(real(eig_Arnodi_H), imag(eig_Arnodi_H),'x');
                    legend('SCinv','LR');
                else
                    legend('SCinv');
                end
                
                xmax = min(max(max(real([eig_Arnodi_H;eig_SCinv])),1.0),3.0);
                ymax = min(max(max(imag([eig_Arnodi_H;eig_SCinv])),1.0),3.0);
                xmin = max(min(min(real([eig_Arnodi_H;eig_SCinv])),-1.0),-3.0);
                ymin = max(min(min(imag([eig_Arnodi_H;eig_SCinv])),-1.0),-3.0);
                %xmax = max(xmax,ymax);
                %ymax = xmax;
                %xx = -xmax:0.01:xmax;
                %yy = -ymax:0.01:ymax;
                %[XX,YY] = meshgrid(xx,yy);
                %ZZ = abs((XX+YY*1i)./(1-XX+YY*1i));
                axis([xmin, xmax, ymin, ymax]);
                %contour(XX,YY,ZZ,[0.2,0.5,1.0,1.5,2.0,5.0,10.0,100.0],'ShowText','on');
                
                title(['Low-rank eigs on level ', num2str(levi), ' rank is ',num2str(local_rank),' out of ', num2str(n-n_e)]);
                %axis image;
                pause(0.01);
            end
